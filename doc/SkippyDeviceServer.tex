...
\large sources version 1.4.2-alpha}
...
When a device of an instance of the \emph{Skippy} device server is launched, the first state it has is \texttt{INIT}, on the left side of the picture \ref{fig:stateMachine}. When all the internal elements are well initialised, then it changes its state to \texttt{OFF}. At this state, the device has an internal object with the responsibility of the communications with an instrument (but no communication has been started).
...
\subsection{Device Properties}\label{sec:devProp}
...
\subsection{Device Commands}\label{sec:devCmds}
...
\subsection{Device Attributes}\label{sec:devAttrs}
...
\subsection{Device features}\label{sec:devFeatures}
