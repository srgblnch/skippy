...
\large sources version 1.6.0-alpha2}
...
\begin{itemize}
    ...
    \item {\tt DynamicAttributes}: optional property to define specific attributes on a single instrument. The syntax is like they can be set in the definition file. If this property is modified with the device alive, there is a command '{\tt updateDynamicAttributes()}' to reload it.
    \item {\tt InstructionsFile}: optional property specify from where, in the file system, the instruction set will be read. When set, the instrument identification will not be used to find the instruction set.
    \item {\tt AvoidIDN}: optional property to avoid the instrument identification. So then, the only way to have dynamic attributes is specifing the file with the '{\tt InstructionsFile}' property or using the '{\tt DynamicAttributes}' property.
\end{itemize}

\subsubsection{Device Commands}\label{sec:devCmds}

Many of those commands changes the device state. See the state machine in figure \ref{fig:stateMachine}.
...
\begin{itemize}
 ...
 seconds}
    \item {\tt \emph{updateDynamicAttributes():}} Expert command to reload, from the content in the '{\tt DynamicAttributes}' property, the dynamic attributes in a single device.
...
\begin{itemize}
    ...
    \item {\tt ReadAfterWrite}: This boolean instructs the behaviour of the device to check if the instrument answers an '{\tt ACK}' after each write.
    \item {\tt Version}: Shows the version string of the running devices.
\end{itemize}
...
See the \href{https://github.com/srgblnch/skippy/issues?q=is\%3Aopen+is\%3Aissue+label\%3Abug}{open issues} that have the label '{\tt bug}' in the github's project.
